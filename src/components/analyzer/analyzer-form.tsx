...
export function AnalyzerForm({ onResult, loading, setLoading }: AnalyzerFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data?.error || t('errors.analysis_failed'));
      }
      ...
      setError(
        err instanceof Error ? err.message : t('errors.analysis_failed')
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          >
            ...
            {loading ? t('form.analyzing') : t('form.analyze_button')}
          </button>
        </div>
      </div>
